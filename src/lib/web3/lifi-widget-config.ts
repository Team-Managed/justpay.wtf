...
import { parseChainIdForWidget, getNativeTokenAddress } from '@/lib/config/network'
 ...
 */
export function buildCheckoutWidgetConfig(opts: CheckoutWidgetOptions): WidgetConfig {
  const hasChainPreference = !!opts.destinationChain
  const hasTokenPreference = !!(opts.destinationTokenSymbol || opts.destinationTokenAddress)
  ...
  const config: WidgetConfig = {
    ...
    ...(opts.amount && Number(opts.amount) > 0 ? { toAmount: opts.amount } : {}),
